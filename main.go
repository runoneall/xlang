...
func RunCompiled(modules *xlang.ModuleMap, data []byte) (err error) {
...
// RunREPL starts REPL.
func RunREPL(modules *xlang.ModuleMap, in io.Reader, out io.Writer) {
	stdin := bufio.NewScanner(in)
	fileSet := parser.NewFileSet()
	globals := make([]xlang.Object, xlang.GlobalsSize)
	symbolTable := xlang.NewSymbolTable()
	for idx, fn := range xlang.GetAllBuiltinFunctions() {
		symbolTable.DefineBuiltin(idx, fn.Name)
	}

	// embed println function
	symbol := symbolTable.Define("__repl_println__")
	globals[symbol.Index] = &xlang.UserFunction{
		Name: "println",
		Value: func(args ...xlang.Object) (ret xlang.Object, err error) {
			var printArgs []any
			for _, arg := range args {
				if _, isUndefined := arg.(*xlang.Undefined); isUndefined {
	...
	for {
		...
		file = addPrints(file)
		c := xlang.NewCompiler(srcFile, symbolTable, constants, modules, nil)
		...
		bytecode := c.Bytecode()
		machine := xlang.NewVM(bytecode, globals, -1)
...
func compileSrc(
	...
	c := xlang.NewCompiler(srcFile, nil, nil, modules, nil)
